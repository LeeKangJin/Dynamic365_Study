...
SELECT		  WRD.OwnerIdName													    AS '담당자'
			...
			, WR.new_name														    AS '주차'
			, W.new_dt_start														AS '시작일(주)'
			, W.new_dt_end															AS '끝일(주)'
			, W.new_p_year															AS '년'
			, W.new_p_month															AS '월'
			, W.new_p_week															AS '주'
...
SELECT 
			  CD.담당자
			 ,CASE WHEN @date BETWEEN CONVERT(DATE, DATEADD(HOUR, 9, CD.시작일(주)), 23) AND CONVERT(DATE, DATEADD(HOUR, 9, W.끝일(주)), 23) THEN '이번주' ELSE '지난주' END AS '주'
			 ,CD.부서
			 ,CD.구분
			 ,CD.완료여부
			 ,CD.대분류
...
FROM #CommonData CD
WHERE 1=1
AND (@date BETWEEN CONVERT(DATE,CD.-15,CD.시작일(주) AND CONVERT(DATE,DATEADD(HOUR,33,CD.끝일(주)),23 )
OR	 @last_date BETWEEN CONVERT(DATE, DATEADD(HOUR, -15, CD.시작일(주)), 23) AND CONVERT(DATE, DATEADD(HOUR, 33, CD.끝일(주)), 23))
